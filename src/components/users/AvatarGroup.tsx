  ...
  return(
    ...
    {avatars.slice(0, maxVisible).map((avatar, index)=>(
      <li key={`avatar-${index}`} className="size-10 rounded-full -ml-3 first:ml-0 group">
        ...
        />
        <span className="absolute hidden group-hover:inline px-2 py-1 rounded text-nowrap font-medium text-xs text-primary-light dark:text-primary-dark bg-primary-dark dark:bg-primary-light">{avatar.name}</span>
      </li>
    ))}
