  ...
  if(active && payload && payload.length) {
    return(
      <div className="bg-primary-light dark:bg-primary-dark shadow-md rounded-lg p-2 border border-quaternary">
        <p className={`mb-1 text-xs font-semibold ${payload[0].name === "Pendiente" && "text-red-light dark:text-red-dark"} ${payload[0].name === "En curso" && "text-yellow-light dark:text-yellow-dark"} ${payload[0].name === "Finaliza" && "text-green-light dark:text-green-dark"}`}>{payload[0].name}</p>
