...
import MovementDetails from "@components/inventory/Details";
  ...
  return(
    ...
    <button type="button" onClick={user && (user.role === "owner" || user.role === "admin") ? ()=>setOpenDetails(true) : ()=>{}} key={movement._id} className={`relative flex items-center gap-3 py-1 px-6 min-h-32 rounded-lg rounded-tl-none bg-secondary-light dark:bg-secondary-dark hover:bg-transparent border-2 border-secondary-light dark:border-secondary-dark ${user && (user.role === "owner" || user.role === "admin") && "cursor-pointer"} group duration-300`}>
      ...
      <div className="flex-1 flex flex-col">
        <p className="text-start font-semibold text-xl text-basic line-clamp-2">{movement.title}</p>
        <p className="text-start font-semibold text-sm text-quaternary line-clamp-1">{movement.description}</p>
        <p className="text-start font-semibold text-sm text-quaternary">{movement.date ? format(movement.date, "dd/MM/yyyy", { locale:es }) : "N/D"}</p>
      </div>
      <div className={`flex flex-col items-center gap-1`}>
        <span className={`px-4 py-0.5 h-fit rounded font-semibold text-sm ${movement.type === "inflow" ? "text-green-light dark:text-green-dark bg-green-light/20 dark:bg-green-dark/20" : "text-red-light dark:text-red-dark bg-red-light/20 dark:bg-red-dark/20"}`}>
          {`${movement.type === "inflow" ? "+" : "-"} ${addThousandsSeparator(movement.quantity)}`}
        </span>
      </div>
      <span className="absolute bottom-full right-0 mb-1 hidden group-hover:flex items-center gap-1 px-5 py-3 rounded-lg bg-primary-light dark:bg-primary-dark border-2 border-secondary-light dark:border-secondary-dark">
        <Image src={movement.createdBy.profileImageUrl || getAvatars()[0]} alt="Avatar" width={100} height={100} className="size-10 rounded-full"/>
        <div className="flex flex-col">
          <p className="font-medium text-sm text-basic">{movement.createdBy.name}</p>
          <p className="font-medium text-xs text-quaternary">{movement.createdBy.email}</p>
        </div>
      </span>
    </button>
    <Modal title={movement.title} isOpen={openDetails} onClose={()=>setOpenDetails(false)}>
      {openDetails && <MovementDetails movement={movement} refresh={refresh}/>}
    </Modal>
    </>
  );
};