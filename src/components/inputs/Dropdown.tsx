...
export default function DropdownSelect({ disabled, label, options, icon, defaultValue, placeholder, handleValue }:{ disabled?:boolean, label?:string, options:{ label:string, value:string }[], defaultValue?:string, icon?:ReactNode, placeholder?:string, handleValue:(value:string)=>void }) {
  ...
  return(
    <div className={`flex-1 relative flex flex-col gap-1 max-h-fit ${disabled && "opacity-50"}`}>
      ...
      <button type="button" onClick={()=>setIsOpen(!isOpen)} disabled={disabled} className="flex justify-between items-center w-full px-3 h-12 rounded-md font-medium text-basic bg-secondary-light dark:bg-secondary-dark focus-within:bg-primary-light dark:focus-within:bg-primary-dark border border-tertiary-light dark:border-tertiary-dark cursor-pointer disabled:cursor-not-allowed duration-300">
