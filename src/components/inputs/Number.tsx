export default function NumberInput({ autoComplete, name, label, placeholder, negative, defaultValue }:{ autoComplete?:string, name:string, label:string, placeholder:string, negative?:boolean, defaultValue?:string }) {
  return(
    <div className="flex flex-col gap-1">
      ...
      <div className="w-full flex items-center justify-between gap-2 px-4  rounded bg-secondary-light focus-within:bg-primary-light dark:bg-secondary-dark dark:focus-within:bg-primary-dark border border-tertiary-light dark:border-tertiary-dark outline-none duration-300">
        ...
        <input id={name} type="number" autoComplete={autoComplete} name={name} placeholder={placeholder} min={1} defaultValue={defaultValue} className="w-full bg-transparent outline-none py-3 font-medium text-basic"/>
      </div>
    </div>
  );
};