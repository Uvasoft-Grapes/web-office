...
export async function PUT(req:NextRequest) {
  try {
    await connectDB();
    const movementId = req.url.split("/")[5].split("?")[0];
    const { title, description, quantity, date, category, status } = await req.json();
...
//! Update Movement
    const currentMovement = await MovementModel.findById(movementId);
    if(!currentMovement) return NextResponse.json({ message:"Transaction not found" }, { status:404 });
...
//! Validations
    if(!movementId) return NextResponse.json({ message:"Missing account" }, { status:400 });
...
//! Update Product
    if(movement.status === "Finalizado") {
      const updatedProduct = await ProductModel.findByIdAndUpdate(movement.product, { $inc:{ stock:movement.type === "inflow" ? -movement.quantity : movement.quantity }, }, { new:true });
      if(!updatedProduct) return NextResponse.json({ message:"Product not found"}, { status:404 });
    };
