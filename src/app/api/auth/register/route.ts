...
//     const { name, email, password, imageUrl, token } = await req.json();
...
//     if(!imageUrl) return NextResponse.json({ message:"Missing image" }, { status:500 });

// //! Check if user already exists
//     const userExists = await UserModel.findOne({ email:formattedEmail });
//     if(userExists) return NextResponse.json({ message:"Introduce un correo electrónico válido" }, { status:400 });
